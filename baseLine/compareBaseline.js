...
let curr_gateway;
 ...
 async function passive(data) {    
    db_getBaseline.get(data.UID).then(()=>{
      ...
      if(data.data.hardware.TotalMemory !== db_getBaseline.baseLine[0].data.TotalMemory){
        onEvent.onEvent(
                    {
                    type: "MEM_SIZE",
                    data: data
  ...
  //  console.log(`${ports.port}`)
    ...
    async function active(data) {    
        db_getBaseline.get(data.UID).then(()=>{
          ...
          if(data.data.networkStats.localLatency > db_getBaseline.baseLine[0].data.LocalLatency){
            onEvent.onEvent(  
              {
              type: "LAT_LOC",
              data: data
          ...
          if(data.data.networkStats.publicLatency > db_getBaseline.baseLine[0].data.PublicLatency){
            onEvent.onEvent(  
              {
              type: "LAT_PUB",
              data: data
          ...
          if(data.data.networkStats.defaultGateway !== db_getBaseline.baseLine[0].data.defaultGatewayy){
